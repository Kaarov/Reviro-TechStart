...
def drf_exception_handler(exc, context):
    ...
    if isinstance(exc, NotAuthenticated):
        return Response(
            {
                "code": ReviroUnauthorized.code,
                "message": ReviroUnauthorized.message,
                "error_fields": [],
            },
            status=ReviroUnauthorized.http_code,
        )

    if isinstance(exc, AuthenticationFailed):
        return Response(
            {
                "code": ReviroAuthenticationFailed.code,
                "message": ReviroAuthenticationFailed.message,
                "error_fields": [],
            },
            status=ReviroAuthenticationFailed.http_code,
        )
...
class ReviroPermissionDenied(ReviroBaseException):
    code = "PermissionDenied"
    message = "Permission Denied"
    http_code = status.HTTP_403_FORBIDDEN


class ReviroUnauthorized(ReviroBaseException):
    code = "Unauthorized"
    message = "Unauthorized"
    http_code = status.HTTP_401_UNAUTHORIZED


class ReviroAuthenticationFailed(ReviroUnauthorized):
